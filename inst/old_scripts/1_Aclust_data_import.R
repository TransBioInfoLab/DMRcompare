...
# Remember that we need to turn off "stringsAsFactors", otherwise this change
#   to a data frame will take a long time, and we'll have to do a lot of
#   conversions back from factors later on. This code originally used the
#   default settings for string conversion.
...
# I don't actually know that this is necessary (and I'm pretty sure it isn't),
#   but until I have the entire workflow coded, I'll change this to a factor to
#   match the "cpg.location.RDS" file from Lily.
...
# Save our results and clean up the workspace. Technically, I can re-compute
#   these results at any time, so I don't need this data. It is only a middle
#   step anyway.
# saveRDS(chrome_annot_files, paste0(data.dir, "annotation-450K-Aclust-by-chromosome.rds"))
...
a <- Sys.time()
cluster_tab <- ConvertCPGList(cpgs_ls = resultsTrim_ls, methylval_df = beta.value.all)
