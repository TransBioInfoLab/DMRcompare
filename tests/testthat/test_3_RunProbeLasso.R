...
runProbeLasso <- function(beta.values, mval_each, r,
                          adjPvalProbe_num,
                          meanLassoRadius_int,
                          minDmrSep_int,
  ...
  type <- factor(sub(".*-", "", colnames(beta.values)))

  # Gabriel O. added the PDFplot = FALSE, Rplot = FALSE options on 2018-06-26
  dmrcoutput <- tryCatch(champ.DMR (beta = as.matrix(beta.values),
                                    pheno = type,
                                    compare.group = NULL,
                                    arraytype = "450K",
                                    method = "ProbeLasso",
  ...
  # extract results if any predicted cluster is identified from DMRcate
  if (!is.null(dmrcoutput)) {
    results.df <- dmrcoutput$ProbeLassoDMR
    ...
    results.df$dmr.pval <- results.df$dmrP
    ...
    #     write.csv(results.df2, file = paste0(result.dir, methods[d], "_sig_dmrs_for_mu", mval_each, "_rep", r, ".csv"), row.names = F)
    # }
...
oldRes <- runProbeLasso(beta.values = treatment_ls$simBetaVals_df,
                        adjPvalProbe_num = 0.001,
                        meanLassoRadius_int = 375,
                        minDmrSep_int = 200,
                        dmr.sig.threshold = 0.05,
                        min.cpgs = 5)

# New function
newRes <- RunProbeLasso(betaVals_mat = treatment_ls$simBetaVals_df,
...
# test
test_that("New RunProbeLasso output equal to legacy output", {
